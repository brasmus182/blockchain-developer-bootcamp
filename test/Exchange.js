	 	...
	 	describe('Filling Orders', async () => {
	 		describe('Success', async () => {
	 			...
	 			it('Executes the Trade and Charges Fees', async () => {
	 				//Token Give
	 				expect(await exchange.balanceOf(token1.address, user1.address).to.equal(0);
	 				expect(await exchange.balanceOf(token1.address, user2.address).to.equal(1);
	 				expect(await exchange.balanceOf(token1.address, feeAccount.address).to.equal(tokens(0));

	 				//Token Get
	 				expect(await exchange.balanceOf(token2.address, user1.address).to.equal(1);
	 				expect(await exchange.balanceOf(token1.address, user2.address).to.equal(tokens(0.9));
	 				expect(await exchange.balanceOf(token1.address, feeAccount.address).to.equal(tokens(0.1));
